...
### Day 19 April 20th 2024
##### 3d Models

**Today's Progress:** Created a component for loading and displaying a gltf model using threejs - this was surprisingly easy, although the results were mediocre at best. I created the model in blender (a program I've just started learning) and I'm still unsure about exactly what is lost when exporting into gltf - I think that blender has a more complicated material system. 

**Thoughts:** I'm working on creating a framework for audio visuals that uses rnbo devices and gltf models, with a schedule that emits device messages and animation commands. Trying to focus on workflows that support specialized tools (like blender), as I have a tendency to try and create everything programmatically. Creative coding is fun challenge but I think that relying on workflows that leverage a more natural interaction with art/music means better results in less time, even if it is harder to automate and replicate.  
